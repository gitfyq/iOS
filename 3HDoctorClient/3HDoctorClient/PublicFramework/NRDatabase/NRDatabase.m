...
{
    static NRDatabase *database;
    static dispatch_once_t once;
    dispatch_once(&once, ^{
        database=[[NRDatabase alloc] init];
    });
    return database;
}
-(instancetype)init
{
    if (self=[super init]) {
        NSString *dbPath = [self copyFile2Documents:@"app_job_app_industy_edu_major_edu_university.db"];
...
-(NSString*) copyFile2Documents:(NSString*)fileName
{
    ...
    if(![fileManager fileExistsAtPath:destPath]){
        NSString* sourcePath =[[NSBundle mainBundle] pathForResource:@"app_job_app_industy_edu_major_edu_university" ofType:@"db"];
